      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2">
          {settings.map((item) => {
            ...
            return (
              <article key={item.title} className={`rounded-3xl border p-5 shadow-xl shadow-slate-950/25 backdrop-blur-xl ${disabled ? "border-white/10 bg-white/5 opacity-70" : "border-white/10 bg-white/5"}`}>
                <div className="flex items-start justify-between gap-3">
                  <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-200">
                    <Icon className="h-5 w-5" />
                  </span>
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300">{item.status}</span>
                </div>
